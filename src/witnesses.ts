...
export const witnessesModule = {
    witnessSchedule: (block: any) => {
        ...
        const rand = parseInt('0x' + hash.substr(hash.length - config.witnessShufflePrecision));
        ...
        const shuffledWitnesses: any[] = [];
        while (witnesses.length > 0) {
            const i = rand % witnesses.length;
    ...
    generateWitnesses: (withWitnessPub: boolean, limit: number, start: number) => {
        ...
        const witnessAccSource = withWitnessPub ? cache.witnesses : cache.accounts;
